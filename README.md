...
6. 成员点击“提交”后，记录会优先写入 Vercel 云数据库；管理员刷新或等待自动刷新即可统计。
...
- Vercel 是否配置了 `DATABASE_URL` 和 `TEAM_SYNC_TOKEN`，并且成员输入的应用密码是否等于 `TEAM_SYNC_TOKEN`。
...
- 配置 `DATABASE_URL` 和 `TEAM_SYNC_TOKEN` 后，成员提交会优先写入 Vercel 云数据库。
