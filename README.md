...
个人默认设置默认展开。当天可分别填写参考成品定额和基准工作时间；当天时间范围0.25至24小时，系统再扣除杂项得到可用工作时间。当日修改随日报保存，不更改个人默认值；新日期继续继承默认设置。

个人报数页面的“个人默认设置”可保存自己的默认参考成品日量和默认基准小时，留空跟随管理员。新日期继承个人默认值；已保存日期保留原时间标准，可点“按最新标准重算当天”更新。当天自定参考成品定额优先于默认日量。默认值随包含它的日报记录同步，因此仍需确认云端写入成功。

项目饱和日量现在跟随后台最新设置，今日、整体预览及参考成品估算共用同一标准；原记录中保存的旧标准不会因查看而被删除，旧日期显示的结果会随项目日量调整。个人基准小时、杂项时间仍使用当天记录。项目数量下显示本次实际采用的日量。
今日页面可直接填写“当天参考成品定额”，按5的倍数向上取整，只影响当前成员当天。这是最终参考值，不再扣一次杂项时间；留空恢复自动估算或管理员参考日量。重算标准不会清除当天自定值。

参考成品定额：默认按当天已做项目比例自动估算，计算 `实际视频成品 / 已完成饱和量 × 当天饱和目标`，向上取5的倍数，例如201显示205。没有产量或缺少项目标准时显示“待估算”。管理员可在每日饱和时间设置里为每人选择自动估算或填写整天参考成品日量；手动日量也按当天剩余时间折算。AI成品不计入视频成品。参考值仅供报数，不参与内部达标判断。
每天保存成品系数及参考日量设置；修改管理员设置后，已保存日期可用“按最新标准重算当天”明确更新。周期与小组参考值按每日、每人相加，缺少任一日估算时不显示不完整总数。

日常达标统一按工作饱和度，数量定额入口已隐藏，旧数值保留用于历史备份。
饱和度低于50%为红色“非常低”，50%至不足80%为黄色“危险”，80%至不足100%为浅绿色“接近达标”，达到100%为绿色“合格/达标”。
每项时间显示占个人基准小时的百分比。新增写代码、兼职本分、沟通工作、看视频、学唱歌、写文章时间，沿用杂项扣减逻辑；管理员可以编辑选项。
缺少项目饱和日量、杂项超过全天或没有可用工作时间时不判定合格。旧记录没有每日时间标准时显示“未结算”；旧格式备份可能仍保留数量定额列，不再据此推断达标。
